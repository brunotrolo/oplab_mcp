...
Servidor MCP (Model Context Protocol) em TypeScript/Express que expõe 35 ferramentas da API REST da OpLab v3, via transporte SSE, hospedado no Google Cloud Run (região `us-east1`).
...
|---|---|---|
| `/health` | GET | Health check. Retorna `{"status":"ok","tools":35,...}`. Usado pelo Cloud Run. |
